...
**根本原因**：某些 JPEG 文件使用特殊编码（如渐进式 JPEG、CMYK 色彩空间等），导致后端 PIL 无法直接识别。

**修复方案** (`src/adapters/serverInpainting.ts`):

- 添加 `convertToPng` 函数，通过 Image + Canvas 重新编码图片
- 所有非 PNG 格式的图片在发送前都转换为 PNG 格式
- 这确保了后端收到的是标准格式的图片数据
- 添加详细的转换日志

### 📝 技术说明

浏览器的 Image 元素能够处理各种特殊编码的图片，通过 Canvas 重新绘制后导出为 PNG，可以确保生成的是标准格式的图片数据。
